...
const handleWithdrawl = async function (amount: number) {
  ...
  await checkbalance(amount).then((res) => {
    ...
    console.log(
      `${chalk.yellow(res)} Withdraw Amount:${amount}\nYour Balance:${
        user1.balance - amount
      }`
    );
  });
};
const handleDeposit = async function (amount: number) {
  // check balance
  await checkbalance(amount).then((res) => {
    user1.balance += amount;
    console.log(
      `${chalk.yellow(res)} Deposited Amount:${amount}\nYour Balance:${
        user1.balance
      }`
    );
  });
...
const handleTransfers = async function (amount: number) {
  ...
  await checkbalance(amount).then((res) => {
    ...
    console.log(
      `${chalk.yellow(res)} Transfer Amount:${amount}\nYour Balance:${
        user1.balance
      }`
    );
  });
...
const checkAmoutIsValid = async (amount: number) => {
  if (Number(amount)) {
    return true;
  } else {
    isValid = false;
