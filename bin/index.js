...
const handleWithdrawl = async function (amount) {
    ...
    await checkbalance(amount)
        ...
        if (res) {
            ...
            spinner.success();
            console.log(`${chalk.yellow("Sucessful")} Withdraw Amount:${amount}\nYour Balance:${user1.balance}`);
        }
    })
        .catch((err) => {
        console.log(err);
...
const handleTransfers = async function (amount) {
    ...
    await checkbalance(amount)
        ...
        if (res) {
            user1.balance -= amount;
            spinner.success();
            console.log(`${chalk.yellow("Sucessful")}\nTransfer Amount:${chalk.yellow(amount)}\nYour Balance:${chalk.yellow(user1.balance)}`);
        }
    })
        .catch((err) => {
        console.log(err);
...
const handleOptions = async function (option) {
    ...
    if (option === "(4) Balance Inquiry") {
        console.log(`${chalk.yellow(`Your Balance: Rs.${chalk.yellow(user1.balance)}`)}\n`);
    }
};
